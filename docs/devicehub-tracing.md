...
**Captures are self-describing.** The action script drives the operator — each
line is `<seconds><TAB><prompt>`. The driver prints the prompt and appends a
marker to the same JSONL stream the tracer writes, so every report lands inside
a labelled window. Nobody has to remember afterwards what they did in which
order, which is how earlier rounds were reconstructed.
...
The driver prints each prompt when it is time to perform it and waits. Idle
steps are part of the measurement: they establish the background report rate
that every other window is compared against.
