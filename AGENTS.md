...
1. Reproduce it with a command from this repo (ideally a `scripts/smoke_matrix.sh` step or a one-line `bin/ipb` invocation) and record host, device, and builds.
...
- The mirror's screen size does **not** come from the decoded frame, which carries encoder padding. It prefers the live primary display nativeSize, then resolves productType (passed in by the wrapper from devicectl) against a built-in table, a runtime Xcode lookup, content detection, then the full frame. Every candidate is range-checked against the current frame. Device orientation drives presentation; native touch and presentation-space pointer mappings are separate. See `docs/video-stream.md` and the 2026-09-09 record in `docs/verification.md`.
